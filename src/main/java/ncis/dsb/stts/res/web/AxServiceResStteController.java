...
@Controller("AxServiceResStteController")
@RequestMapping("/dsb/stts/res/axServiceResStte")
public class AxServiceResStteController extends BaseController {


	@Resource(name="axServiceResStteService")
	AxServiceResStteService axServiceResStteService;

	/**
	 * 자원확장 서비스별 자원현황 목록 조회
	 * @param request
	 * @param model
	 * @param searchVo
	 * @return
	 * @throws Exception
	 */
	@RequestMapping(value="/selectAxServiceResStteList.do")
	public String selectAxResStteList( AxServiceResSearchVo searchVo,
			HttpServletRequest request,
			Model model) throws Exception{

		List<AxServiceResStteVo> list = null;
		if(searchVo.getSearchTrmCd()!=null){
			list = axServiceResStteService.selectAxServiceResStteList(searchVo);
		...
		return dashTilesView(request,"selectAxServiceResStteList");
	}
	/**
	 * 서비스별 시간대별 자원사용률 팝업
	 * @param request
	...
	@RequestMapping(value="/selectAxServiceTimeResUseRtP.do")
	public String selectAxServiceTimeResUseRtP(
			AxServiceResSearchVo searchVo,
			HttpServletRequest request,
			Model model) throws Exception{
		...
		List<Map<String,String>> podList = axServiceResStteService.selectAxServiceResSttePodList(searchVo);
		...
		List<Map<String,String>> cpuList = axServiceResStteService.selectAxServiceTimeResUseRtPivot(searchVo);
		searchVo.setColumn("MEM");
		List<Map<String,String>> memList = axServiceResStteService.selectAxServiceTimeResUseRtPivot(searchVo);
		searchVo.setColumn("NETIN");
		List<Map<String,String>> netInList = axServiceResStteService.selectAxServiceTimeResUseRtPivot(searchVo);
		searchVo.setColumn("NETOUT");
		List<Map<String,String>> netOutList = axServiceResStteService.selectAxServiceTimeResUseRtPivot(searchVo);
		model.addAttribute("searchVo", searchVo);
		model.addAttribute("yearCdList", DsbUtil.getYearCd(7));
		...
		model.addAttribute("title", "서비스별 시간대별 자원사용률");


		return popup(request,"selectAxServiceTimeResUseRtP");
	}

	/**
	 * 자원확장 서비스별 자원현황 엑셀다운로드
	 * @param request
	 * @param model
	 * @param searchVo
	 * @return
	...
	@RequestMapping(value="/selectAxServiceResStteListXlsDown.do")
	public void selectAxServiceResStteListXlsDown( AxServiceResSearchVo searchVo,
			HttpServletRequest request,
		...
		Sheet sheet = workBook.createSheet("자원확장 서비스별 자원현황");
		...
		Cell titleCell5 = row.createCell(4);	titleCell5.setCellValue( "서비스명" );						titleCell5.setCellStyle(headerStyle);
		...
		titleCell5 = row.createCell(4);			titleCell5.setCellValue( "서비스명" );						titleCell5.setCellStyle(headerStyle);
		...
		sheet.addMergedRegion(new CellRangeAddress(0,2,4,4));//서비스명
		...
		if(list==null){
			...
			list = new ArrayList<AxServiceResStteVo>();
		}
		for(int i=0; i<list.size(); i++){
			AxServiceResStteVo vo = list.get(i);
			...
			Cell dataCell4 = dataRow.createCell(4);	dataCell4.setCellValue(DateUtil.stringValueOf(vo.getServcNm()));
        ...
        response.setHeader("Content-Disposition", "attachment; filename=\"" + new String(("자동확장 서비스별 자원현황").getBytes("KSC5601"),"8859_1")+"_"+DateUtil.getCurrentDate() + ".xlsx\"");
	...
	/**
	 * 서비스별 시간대별 자원사용률 엑셀다운로드
	 * @param request
	 * @param model
	 * @return
	 * @throws Exception
	 */
	@RequestMapping(value="/selectAxServiceTimeResUseRtPXlsDown.do")
	public void selectAxServiceTimeResUseRtPXlsDown( AxServiceResSearchVo searchVo,
			HttpServletRequest request,
			HttpServletResponse response,
			Model model) throws Exception{
		if("DD".equals(searchVo.getSearchTrmCd()) && StringUtils.isEmpty(searchVo.getColctCd())){
		...
		OutputStream out = response.getOutputStream();
        response.setContentType("application/vnd.ms-excel");
        response.setHeader("Content-Disposition", "attachment; filename=\"" + new String(("서비스별 시간대별 자원사용률").getBytes("KSC5601"),"8859_1")+"_"+DateUtil.getCurrentDate() + ".xlsx\"");
