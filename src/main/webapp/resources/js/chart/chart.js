...
(function(){
	...
	Chart.defaults = {
		global: {
			...
			multiTooltipTemplate: " <%= value %>",//값만 보여달라는 요청에 의해 수정
...
(function(){
	"use strict";
	var barMaxWidth=100;// 막대차트 max width 설정
	var root = this,
		Chart = root.Chart,
			...
			helpers.each(this.datasets,function(dataset,datasetIndex){
				helpers.each(dataset.bars,function(bar,index){
					if (bar.hasValue()){
						...
						if(bar.transition().width>barMaxWidth){bar.transition().width=barMaxWidth}//막대차트 width 최대갑 설정
						bar.transition().draw();
					}
				},this);
...
(function(){
	...
	Chart.Type.extend({
		...
		initialize:  function(data){
			...
			//Set up tooltip events on the chart
			if (this.options.showTooltips){
				var tmpPointDot=this.options.pointDot;
				helpers.bindEvents(this, this.options.tooltipEvents, function(evt){
					...
					this.eachPoints(function(point){
						if(tmpPointDot){
							...
							point.restore(['fillColor', 'strokeColor','display']);
						}

					});
					helpers.each(activePoints, function(activePoint){
						activePoint.display=false;
						activePoint.display=true;
