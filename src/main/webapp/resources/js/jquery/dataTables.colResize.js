    ...
    var factory = function ($, DataTable) {
        ...
        var ColResize = function (dt, opts) {
            ...
            this.s = {
                 ...
                 */
                "isMousedown": false,
                "colsize_first": true /* first column sizing */
            };
...
/*
            var that = this;
            $(this.s.dt.nTable).children().find("tr:first-child > td").each(function(idx) {

            	var clientWidth = $(this)[0].clientWidth +  "px";
            	that.s.dt.aoColumns[ idx ].sWidth = clientWidth;
            	that.s.dt.aoColumns[ idx ].sWidthOrig = clientWidth;
...
*/
        ...
        ColResize.prototype = {
            ...
            "_fnMouseDown": function (e, nTh) {
                ...
                if( this.s.colsize_first ) {
                	/**
                	 * 초기 셋티에 따른 사이즈 재구성 처리....
                	 * 힘들다..ㅜㅜ
                	 */
                	...
                	var tableWidth = target.parents("table").width();
                	//target.parent().children().each( function(idx) {
                	$(that.s.dt.aoColumns).each(function(idx) {
                		totalWidth += parseInt(that.s.dt.aoColumns[idx].sWidth);
                	});

                	var colRate = tableWidth / totalWidth;
                	//target.parent().children().each( function(idx) {
                	$(that.s.dt.aoColumns).each(function(idx) {
                		clientWidth = parseInt(that.s.dt.aoColumns[idx].sWidth) * colRate;
                		that.s.dt.aoColumns[idx].nTh.style.width = clientWidth + "px";
                		that.s.dt.aoColumns[idx].sWidth = clientWidth + "px";
                		that.s.dt.aoColumns[idx].sWidthOrig = clientWidth + "px";
                		//console.log(idx + ' = ' + clientWidth);
                	});

	                this.s.colsize_first = false; // first
                }
                ...
                //if(target.attr('data-column-index')>idx) idx = target.attr('data-column-index');
                //console.log(idx);
            ...
            "_fnMouseMove": function (e) {
                ...
                if(typeof that.s.mouse.targetColumn == 'undefined') return;
...
//                console.log( relativeX );
                ...
                var minColumnWidth = Math.max(parseInt($(that.s.mouse.targetColumn.nTh).css('min-width')), 30); //30
                ...
                if ((dx > 0 && distFromRight <= 0) || (dx < 0 && distFromRight >= 0)) {
                    ...
                    if (!that.s.init.tableWidthFixed) {
                        ...
                        var fixedFlag = domCols.parents("table").hasClass('table-layout-fixed');
                        /**
                    	if(fixedFlag && domCols.parent().parent().find('th[colspan]').filter(function(){ return parseInt($(this).attr('colspan')) > 1;}).length>0) {
                    		//domCols.parents("table").removeClass('table-layout-fixed');
                    	} */
                        var preTbWidth;
                        domCols.parents("table").each(function() {
                            if(!$(this).parent().hasClass("DTFC_LeftBodyLiner")) {
                            	preTbWidth = $(this).width();
                        ...
                        // table, layout-fixed 가 아닌 경우, colspan 컬럼에 대한 처리 버그 보완, 2017-09-27
                        if (!fixedFlag && widthDiff < 0 && domCols.width() > newColWidth) {
                        	domCols.parents("table").width(preTbWidth);
                        	that.s.mouse.targetColumn.sWidthOrig = that.s.mouse.targetColumn.sWidth = that.s.mouse.targetColumn.width = prevWidth + "px";
                        	domCols.width(that.s.mouse.targetColumn.width);
                        ...
                        /**
                        if(fixedFlag && domCols.parent().parent().find('th[colspan]').filter(function(){ return parseInt($(this).attr('colspan')) > 1;}).length>0) {
                    		domCols.parents("table").addClass('table-layout-fixed');
                    	} */
                    } else {
                    	console.log( "tableWidthFixed.........." );
