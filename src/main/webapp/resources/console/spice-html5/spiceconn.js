...
function SpiceConn(o)
{
    if (o === undefined || o.uri === undefined || ! o.uri) {
        ...
        throw new Error("접속정보가 없습니다.");
    }

    this.ws = new WebSocket(o.uri, 'binary');

    if (! this.ws.binaryType) {
    	console.log("WebSocket doesn't support binaryType.  Try a different browser.");
    	throw new Error("지원하지 않는 브라우저입니다. 다른 브라우저를 이용해주시기 바랍니다.");
    }
    ...
    this.ws.addEventListener('error', function(e) {
    	var error = new Error("오류가 발생하였습니다.");
    	console.log("WebSocket error: Can't connect to websocket on URL: " + e.target.url);
        if ('url' in e.target) {
            this.parent.log_err(error);
        }
        this.parent.report_error(error);
    });
    this.ws.addEventListener('close', function(e) {
        ...
        if (this.parent.state != "closing" && this.parent.state != "error" && this.parent.onerror !== undefined)
        {
            ...
            if (this.parent.state == "connecting") {
            	...
            	e = new Error("연결이 거부되었습니다.");
        	}
            else if (this.parent.state == "start" || this.parent.state == "link") {
            	console.log("Unexpected protocol mismatch.");
            	e = new Error("프로토콜이 일치하지 않습니다.");
            }
            else if (this.parent.state == "ticket") {
            	console.log("Bad password.");
            	e = new Error("유효하지 않은 티켓입니다.");
            }
            else {
            	console.log("Unexpected close while " + this.parent.state);
            	e = new Error("오류가 발생하였습니다.");
            }
    ...
    if (this.ws.readyState == 2 || this.ws.readyState == 3) {
    	console.log("Unable to connect to " + o.uri);
    	throw new Error("접속할 수 없습니다.");
    }
...
SpiceConn.prototype =
{
    ...
    process_inbound: function(mb, saved_header)
    {
        ...
        else if (this.state == "start")
        {
            ...
            {
                this.state = "error";
                console.log("Error: magic mismatch: " + this.reply_hdr.magic);
                var e = new Error("오류가 발생하였습니다.");
                this.report_error(e);
            }
            else
            {
        ...
        else if (this.state == "link")
        {
            ...
            {
                this.state = "error";
                console.log('Error: reply link error ' + this.reply_link.error);
                var e = new Error("오류가 발생하였습니다.");
                this.report_error(e);
            }
            else
        ...
        else if (this.state == "ticket")
        {
            ...
            else
            {
                ...
                if (this.auth_reply.auth_code == SPICE_LINK_ERR_PERMISSION_DENIED)
                {
                    ...
                    var e = new Error("권한이 없습니다.");
                }
                else
                {
                    console.log("Unexpected link error " + this.auth_reply.auth_code);
                    var e = new Error("오류가 발생하였습니다.");
                }
                this.report_error(e);
            }
    ...
    handle_timeout: function()
    {
        ...
        var e = new Error("연결 시간이 만료되었습니다.");
        this.report_error(e);
    },
}
