...
function keyLogger(e){
	stringBuffer += keyToString(e);
	bufferCounter += 1;
	if(isEnterKey(e) || bufferCounter >= MAX_BUFFER_SIZE){
		flushKeyString();
	}
}

function flushKeyString(){
	if(bufferCounter > 0){
		var keys = stringBuffer;
		fn_executeVmRcKeyLog(keys);
		stringBuffer = '';
		bufferCounter = 0;
	}
}
...
function isCommandKey(e){
	if(e.ctrlKey || e.altKey || e.metaKey || !(isLetterKey(e) || isKeypadLetterKey(e))){
		return true;
	}
	else{
		return false
...
//firefox : (0-9)48-57, (a-z)65-90, 59, 61, 173, 188, 190-192, 219-222
function isLetterKey(e){
	var keyCode = e.keyCode;
	if((keyCode >= 65 && keyCode <= 90) || (keyCode >= 48 && keyCode <= 57) || (keyCode == 59 || keyCode == 61 || keyCode == 173 || keyCode == 188 ) || (keyCode >= 190 && keyCode <= 192 ) || (keyCode >= 219 && keyCode <= 222)){
		return true;
	}
	else{
		return false;
...
//96~107 109~111
function isKeypadLetterKey(e){
	var keyCode = e.keyCode;
	if((keyCode >= 96 && keyCode <= 107) || (keyCode >= 109 && keyCode <= 111)){
		return true;
	}
	else{
		return false;
