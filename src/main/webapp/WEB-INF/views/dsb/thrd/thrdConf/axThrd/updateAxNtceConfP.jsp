				...
				<tbody>
					<tr>
						...
						<td><c:forEach var="vo" items="${grdCode }" varStatus="i">
								...
								<label for="dspthGrdCdList${vo.cd}">${vo.cdNm }</label><br>
							</c:forEach>
						</td>
					</tr>
					<tr>
...
<script>
	function fn_searchUser() {
		$.post(
						'<c:url value="/dsb/thrd/thrdConf/axThrd/selectUserList.do"/>',
						{
	...
	//검색조건 초기화
	function fn_reset() {
		jConfirm("통보설정을 초기화하시겠습니까?", function() {
			var url = '<c:url value="deleteNtceConf.do" />';
			$.post(url, {
