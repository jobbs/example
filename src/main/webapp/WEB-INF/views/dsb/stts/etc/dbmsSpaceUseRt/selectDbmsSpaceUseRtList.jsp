			...
			<c:forEach var="date" items="${dateList }" varStatus="k">
				...
				<c:forEach var="vo" items="${list }" varStatus="j">
					<c:if test="${vo.objName eq job and date eq vo.checkDatetime}">
						...
						<c:out value='${vo.dbBufferCacheAvg}'/>
					</c:if>
				</c:forEach>
				<c:if test="${dateCnt==0}">
				<c:set var="objCnt" value="${objCnt+1}"/>
			...
			<c:forEach var="date" items="${dateList }" varStatus="k">
				...
				<c:forEach var="vo" items="${list }" varStatus="j">
					<c:if test="${vo.objName eq job and date eq vo.checkDatetime}">
						...
						<c:out value='${vo.dbSpaceUseRtAvg}'/>
					</c:if>
				</c:forEach>
				<c:if test="${dateCnt==0}">
				<c:set var="objCnt" value="${objCnt+1}"/>
...
function setJob(evt) {
	...
	fn_searchDbmsServer(job.jobId);
}
function fn_searchDbmsServer(jobId){
	var data={
			'managetypeId' : 'DBMS',
			'jobId' : jobId
	};
...
function fn_setDbmsServer(dbmsList){
	$('#objId option').remove();//삭제
	var option ='<option value="">전체</option>';
	$(option).appendTo($('#objId'));
	for(var i=0; i<dbmsList.length; i++){
		var option ='<option value="'+dbmsList[i].objId+'">'+dbmsList[i].objName+'</option>';
		$(option).appendTo($('#objId'));
	}
}
...
function fn_search(){
	...
	searchVo.action='<c:url value="selectDbmsSpaceUseRtList.do" />';
	searchVo.submit();
}
			...
			<div class="box-body collapse in search-collapse">
				<div class="form-group">
					...
					<div class="form-cell form-cell-50 col-lay-25">
						...
						<div class="cell-body">
							<form:select cssClass="form-control input-sm" path="objId" title="DBMS">
								<form:option value="">전체</form:option>
								<c:forEach var="vo" items="${dbmsList}" varStatus="i">
...
<div class="col-box-100 detail-col">
	<div class="col-cell-group clearfix">
		...
		<div class="col-cell-50 no-padding-right">
			<div class="box">
				<div class="box-header">
					<h3 class="box-title">버퍼캐시 적중률</h3>
				</div>
				<!-- /box-header -->
	...
	<div class="box list-box">
		...
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical" id="table">
				<caption>(DBMS)버퍼캐쉬 적중률 목록</caption>
				<colgroup>
					<col class="col25">
					<col class="col25">
					<col class="col25">
						...
						<c:otherwise>
							<c:forEach var="vo" items="${list }" varStatus="i">
								<tr>
									...
									<td class="alignR"><fmt:formatNumber value="${vo.dbBufferCacheAvg}" type="number" pattern="#,##0.00" /></td>
									<td class="alignR"><fmt:formatNumber value="${vo.dbSpaceUseRtAvg}" type="number" pattern="#,##0.00" /></td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
...
function excelDown(){
	...
	searchVo.action='<c:url value="selectDbmsSpaceUseRtXlsDown.do" />';
	searchVo.submit();
}
</script>