					...
					<c:forEach var="date" items="${dateList }" varStatus="k">
						...
						<c:forEach var="vo" items="${list }" varStatus="j">
							<c:if test="${vo.objName eq job and date eq vo.checkDatetime}">
								...
								<c:out value='${vo.maxCncrtUsertQty}'/>
							</c:if>
						</c:forEach>
						<c:if test="${dateCnt==0}">
						<c:set var="objCnt" value="${objCnt+1}"/>
			...
			<c:forEach var="date" items="${dateList }" varStatus="k">
				...
				<c:forEach var="vo" items="${list }" varStatus="j">
					<c:if test="${vo.objName eq job and date eq vo.checkDatetime}">
						...
						<c:out value='${vo.maxDailyVstrQty}'/>
					</c:if>
				</c:forEach>
				<c:if test="${dateCnt==0}">
				<c:set var="objCnt" value="${objCnt+1}"/>
...
function setJob(evt) {
	...
	fn_searchWasServer(job.jobId);
}
function fn_searchWasServer(jobId){
	var data={
			'managetypeId' : 'WAS',
			'jobId' : jobId
	};
...
function fn_search(){
	...
	searchVo.action='<c:url value="selectWasSmTimeConectList.do" />';
	searchVo.submit();
}
		...
		<div class="box box-search">
			...
			<div class="box-body collapse in search-collapse">
				<div class="form-group">
					...
					<div class="form-cell form-cell-50 col-lay-25">
						<div class="cell-title">
							<label>WAS SERVER</label>
						</div>
						<div class="cell-body">
							<form:select cssClass="form-control input-sm" path="objId" title="WAS SERVER">
								<form:option value="">전체</form:option>
								<c:forEach var="vo" items="${wasList}" varStatus="i">
...
<div class="col-box-100 detail-col">
	<div class="col-cell-group clearfix">
		...
		<div class="col-cell-50 no-padding-right">
			<div class="box">
				<div class="box-header">
					<h3 class="box-title">최대 동시접속자 수</h3>
				</div>
				<!-- /box-header -->
...
<div class="col-box-100 search-col">
	<div class="box list-box">
		<div class="box-header">
			<h3 class="box-title">
				최대 동시접속자 수 및 방문자 수 목록 <small> </small>
			</h3>
			<div class="box-tools">
				<div class="input-group-box">
		...
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical" id="table">
				<caption>(WAS)일일방문자 수 목록</caption>
				<colgroup>
					<col class="col25">
					<col class="col25">
					<col class="col25">
						...
						<c:otherwise>
							<c:forEach var="vo" items="${list }" varStatus="i">
								<tr>
									...
									<td class="alignR"><fmt:formatNumber value="${vo.maxCncrtUsertQty}" type="number" pattern="#,##0" /></td>
									<td class="alignR"><fmt:formatNumber value="${vo.maxDailyVstrQty}" type="number" pattern="#,##0" /></td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
...
function excelDown(){
	...
	searchVo.action='<c:url value="selectWasSmTimeConectXlsDown.do" />';
	searchVo.submit();
}
</script>