	...
	<div class="box list-box">
		...
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical">
				...
				<tbody>
					<c:choose>
						<c:when test="${cpuUseRtList eq null or empty cpuUseRtList }">
						...
						<c:otherwise>
							...
							<c:forEach var="vo" items="${cpuUseRtList }" varStatus="status1">
								<tr>
									<c:if test="${status1.index == 0 }">
										<td rowspan="10">vCore</td>
									</c:if>
	...
	<div class="box list-box">
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical">
				...
				<tbody>
					<c:choose>
						<c:when test="${topCpuUseRtList eq null or empty topCpuUseRtList }">
							<c:if test='${searchVo.regionId eq null }'>
								<tr>
									<td colspan="14">조회 버튼을 클릭하시기 바랍니다.</td>
								</tr>
						...
						<c:otherwise>
							<c:forEach var="vo" items="${topCpuUseRtList }" varStatus="status">
								<tr>
...
function fn_search() {
	...
	$("#frm").attr("action", "<c:url value='selectCpuUseRtList.do'/>");
	$("#frm").submit();
	return false;
}

function excelDown(){
	if(${cpuUseRtList eq null or empty cpuUseRtList}) {
		jAlert("엑셀로 다운로드할 데이터가 없습니다.");
		return;
	}

	$("#frm").attr("target", "_self");
	$("#frm").attr("action", "<c:url value='selectCpuUseRtXlsDown.do'/>");
	$("#frm").submit();
	return false;
}
</script>