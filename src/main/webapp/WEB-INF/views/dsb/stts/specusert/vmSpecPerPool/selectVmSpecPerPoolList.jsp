	...
	<div class="box list-box">
		...
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical">
				<caption>자원풀별 가상서버 사양 목록</caption>
				<colgroup>
					<col class="col5">
					<col class="colAuto">
					<col class="col9">
				...
				<thead>
					<tr>
						<th rowspan="2">No.</th>
						<th rowspan="2">자원풀</th>
						<th colspan="3">vCore</th>
						<th colspan="3">MEM(GB)</th>
						<th colspan="3">SAN(GB)</th>
					</tr>
					<tr>
						<th>최저</th>
						<th>평균</th>
						...
						<c:otherwise>
							<c:forEach var="vo" items="${list }" varStatus="i">
								<tr>
									...
									<td class="alignR"><c:out value="${vo.minAvgVcoreQty }" /></td>
									<td class="alignR"><c:out value="${vo.avgAvgVcoreQty }" /></td>
									<td class="alignR"><c:out value="${vo.maxAvgVcoreQty }" /></td>
									<td class="alignR"><c:out value="${vo.minMemSumCapa }" /></td>
									<td class="alignR"><c:out value="${vo.avgMemSumCapa }" /></td>
									<td class="alignR"><c:out value="${vo.maxMemSumCapa }" /></td>
									<td class="alignR"><c:out value="${vo.minStrgSumCapa }" /></td>
									<td class="alignR"><c:out value="${vo.avgStrgSumCapa }" /></td>
									<td class="alignR"><c:out value="${vo.maxStrgSumCapa }" /></td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
...
<script type="text/javascript">
	function goPage(page) {
		location.href = "selectVmSpecPerPoolList.do?paginationInfo.currentPageNo="
				+ page + "&${listParam}";
	}
	...
	function fn_search() {
		...
		$("#frm").attr("action", "<c:url value='selectVmSpecPerPoolList.do'/>");
		$("#frm").submit();
		return false;
	}
	...
	function excelDown() {
		...
		$("#frm").attr("target", "_self");
		$("#frm").attr("action",
				"<c:url value='selectVmSpecPerPoolXlsDown.do'/>");
		$("#frm").submit();
		return false;
	}
