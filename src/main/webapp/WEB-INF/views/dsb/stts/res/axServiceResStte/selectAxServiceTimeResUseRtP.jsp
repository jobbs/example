...
function ChartDraw() {
		...
		<c:forEach var="podItem" items="${podList }">
			<c:forEach var="cpuItem" items="${cpuList }">
				<fmt:formatNumber var="useRt" type="number" value="${cpuItem[podItem.pod_id]}"/>
				<c:choose>
			...
			<c:forEach var="memItem" items="${memList }">
				<fmt:formatNumber var="useRt" type="number" value="${memItem[podItem.pod_id]}"/>
				<c:choose>
			...
			<c:forEach var="netInItem" items="${netInList }">
				<c:set var="useRt" value="${netInItem[podItem.pod_id]}"/>
				<c:choose>
			...
			<c:forEach var="netOutItem" items="${netOutList }">
				<c:set var="useRt" value="${netOutItem[podItem.pod_id]}"/>
				<c:choose>
    ...
    var cpuAreaChartData = {
      ...
      datasets: [
		<c:forEach var="podItem" items="${podList }" varStatus="i">
			<c:if test="${i.count>1}">
				,
			</c:if>
				{
			          label:"<c:out value='${podItem.pod_nm}'/>",
			          data: [
							<c:forEach var="cpuItem" items="${cpuList }" varStatus="j">
								<c:if test="${j.count>1}">	,		</c:if>
								<fmt:formatNumber var="useRt" type="number" value="${cpuItem[podItem.pod_id]}"/>
								<c:choose>
									<c:when test="${empty cpuItem[podItem.pod_id]}">""</c:when>
    					...
    					</c:if>
    						{
    					          ...
    					          data: [
    									<c:forEach var="memItem" items="${memList }" varStatus="j">
    										...
    										<fmt:formatNumber var="useRt" type="number" value="${memItem[podItem.pod_id]}"/>
    										<c:choose>
    											<c:when test="${empty memItem[podItem.pod_id]}">""</c:when>
    											<c:otherwise><c:out value='${useRt}'/></c:otherwise>
    										</c:choose>
					...
					</c:if>
						{
					          ...
					          data: [
									<c:forEach var="netInItem" items="${netInList }" varStatus="j">
										...
										<c:set var="useRt" value="${netInItem[podItem.pod_id]}"/>
										<c:choose>
											<c:when test="${empty netInItem[podItem.pod_id]}">""</c:when>
											<c:otherwise><c:out value='${useRt}'/></c:otherwise>
										</c:choose>
					...
					</c:if>
						{
					          ...
					          data: [
									<c:forEach var="netOutItem" items="${netOutList }" varStatus="j">
										...
										<c:set var="useRt" value="${netOutItem[podItem.pod_id]}"/>
										<c:choose>
											<c:when test="${empty netOutItem[podItem.pod_id]}">""</c:when>
											<c:otherwise><c:out value='${useRt}'/></c:otherwise>
										</c:choose>
			...
			<div class="box-body ">
				<div class="form-group">
		           <div class="form-cell form-cell-25">
			           ...
			           <div class="cell-body">
		             		<form:input path="servcNm" cssClass="form-control input-sm" maxlength="" title="서비스명" readonly="true"/>
		             	</div>
				   </div>
...
<div class="col-box-100 detail-col">
        ...
        <div class="box border-gray">
            ...
            <div class="box-tools">
              <div class="input-group-box">
        		<div class="input-group-cell pad-right">
					...
					<!-- 접기 버튼 -->
					<button type="button" class="btn btn-sm collapsed" data-toggle="collapse" data-target=".search-collapse">
          ...
          <div class="box-body no-padding collapse search-collapse">
              <table class="table table-hover table-vertical table-bordered table-layout-fixed">
              <caption>CPU 사용률</caption>
                <colgroup>
                	<col class="col50">
                		<c:if test="${podList eq null or empty podList }">
							<script>
								$('canvas').hide();
							</script>
						</c:if>
						<c:forEach var="vo" items="${podList }" varStatus="i">
							<col class="col40">
                  		</c:forEach>
                </colgroup>
                <thead>
                	<tr>
                		<th>기간</th>
                  		<c:forEach var="vo" items="${podList }" varStatus="i">
							<th title="<c:out value="${vo.pod_nm}"/>"><c:out value="${vo.pod_nm}"/></th>
                  		</c:forEach>
                	</tr>
                </thead>
                <tbody>
                ...
                <c:forEach var="map" items="${cpuList }" varStatus="i">
                	<tr>
		              ...
		              <c:forEach var="podItem" items="${podList }">
		              	<c:set var="podId" value="${podItem.pod_id}"/>
        ...
        <div class="box">
          <div class="box-header">
            ...
            <div class="box-tools">
              <div class="input-group-box">
        		<div class="input-group-cell pad-right">
					<!-- 접기 버튼 -->
					<button type="button" class="btn btn-sm collapsed" data-toggle="collapse" data-target=".mem-search-collapse">
						<i class="fa fa-chevron-up" data-toggle="tooltip" title="" data-original-title="접어두기"></i>
					</button>
        	    </div>
              </div>
          ...
          <div class="box-body no-padding collapse mem-search-collapse">
              <table class="table table-hover table-vertical table-bordered table-layout-fixed">
        ...
        <div class="box">
          <div class="box-header">
            ...
            <div class="box-tools">
              <div class="input-group-box">
        		<div class="input-group-cell pad-right">
					<!-- 접기 버튼 -->
					<button type="button" class="btn btn-sm collapsed" data-toggle="collapse" data-target=".net-in-search-collapse">
						<i class="fa fa-chevron-up" data-toggle="tooltip" title="" data-original-title="접어두기"></i>
					</button>
        	    </div>
              </div>
          ...
          <div class="box-body no-padding collapse net-in-search-collapse">
              <table class="table table-hover table-vertical table-bordered table-layout-fixed">
        ...
        <div class="box">
          <div class="box-header">
            ...
            <div class="box-tools">
              <div class="input-group-box">
        		<div class="input-group-cell pad-right">
					<!-- 접기 버튼 -->
					<button type="button" class="btn btn-sm collapsed" data-toggle="collapse" data-target=".net-out-search-collapse">
						<i class="fa fa-chevron-up" data-toggle="tooltip" title="" data-original-title="접어두기"></i>
					</button>
        	    </div>
              </div>
          ...
          <div class="box-body no-padding collapse net-out-search-collapse">
              <table class="table table-hover table-vertical table-bordered table-layout-fixed">
...
<script>
      ...
      function excelDown(){
    	 <c:if test="${podList eq null or empty podList }">
    		...
    		searchVo.action='<c:url value="selectAxServiceTimeResUseRtPXlsDown.do" />';
    		searchVo.submit();
    	}

      function fn_search(){
   		searchVo.action='<c:url value="selectAxServiceTimeResUseRtP.do" />';
   		searchVo.submit();
   		return false;
   		}
</script>