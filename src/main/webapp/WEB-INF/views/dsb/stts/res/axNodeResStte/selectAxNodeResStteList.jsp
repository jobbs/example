...
<script>
	...
	function fn_search() {
		...
		searchVo.action = '<c:url value="selectAxNodeResStteList.do" />';
		searchVo.submit();
	}

	function fn_openAxNodeTimeResUseRt(rsrcPoolId, rsrcPoolNm, atmsclNodeId, atmsclNodeNm) {
		...
		params.rsrcPoolId = rsrcPoolId;
		params.rsrcPoolNm = rsrcPoolNm;
		params.atmsclNodeId = atmsclNodeId;
		params.atmsclNodeNm = atmsclNodeNm;
		//alert(params.jobNm);
		popup('<c:url value="selectAxNodeTimeResUseRtP.do" />' + '?'
				+ $.param(params), 'AxNodeTimeResUseRt', 1200, 1000);
	}
</script>
						...
						<c:otherwise>
							<c:forEach var="vo" items="${list}" varStatus="ri">
								<tr>
									...
									<td class="ellipsis alignL"><a href="javascript:fn_openAxNodeTimeResUseRt('<c:out value="${vo.rsrcPoolId }"/>','<c:out value="${vo.rsrcPoolNm }"/>')"><c:out value="${vo.rsrcPoolNm }" /></a></td>
									<td class="alignL notellipsis"><a href="javascript:fn_openAxNodeTimeResUseRt('<c:out value="${vo.rsrcPoolId }"/>','<c:out value="${vo.rsrcPoolNm }"/>','<c:out value="${vo.atmsclNodeId }"/>','<c:out value="${vo.atmsclNodeNm }"/>')"><c:out value="${vo.atmsclNodeNm }" /></a></td>
...
<c:choose>
	<c:when test="${ (list eq null or empty list) and (searchVo.search eq null or empty searchVo.search) }">
		emptymsg = "조회 버튼을 클릭해 주시기 바랍니다.";
	</c:when>
</c:choose>

		$('#AxNodeResStteTable colgroup').remove();
		var table = $('#AxNodeResStteTable').DataTable( {
			dom: 'Zlfrtip',
			paging : false,
			searching : false,
			info : false,
	...
	function goPage(page) {
		location.href = "selectAxNodeResStteList.do?paginationInfo.currentPageNo="
				+ page + "&${listParam}";
	}
	...
	function fn_excelDown() {
		...
		searchVo.action = '<c:url value="selectAxNodeResStteListXlsDown.do" />';
		searchVo.submit();
	}
