...
function ChartDraw() {
		...
		var sanValArr =[];
		var netValArr =[];
		var maxCpuUseRt=0,maxMemUseRt=0, maxSanUseRt=0, maxNetUseRt=0;
		<c:forEach var="vmItem" items="${vmList }">
			...
			<c:forEach var="netItem" items="${netList }">
				<c:set var="useRt" value="${netItem[vmItem.vm_seq]}"/>
				<c:choose>
					<c:when test="${empty useRt}">netValArr.push(0);</c:when>
					<c:otherwise>netValArr.push(<c:out value='${useRt}'/>);</c:otherwise>
				</c:choose>
			</c:forEach>
		</c:forEach>
      ...
      datasets: [
		<c:forEach var="vmItem" items="${vmList }" varStatus="i">
			<c:if test="${i.count>1}">
				...
				{
			          label:"<c:out value='${vmItem.vm_comp_id}'/>",
			          data: [
    ...
    var netAreaChartData = {
    		labels: [
		               	<c:forEach var="netItem" items="${netList }" varStatus="i">
		               		<c:if test="${i.count>1}">,</c:if>
		               		<c:choose>
		               			<c:when test='${searchVo.colctCd eq "MI" or searchVo.colctCd eq "HH"}'>
		               			"${fn:substringAfter(netItem.dt,' ')}"
		               			</c:when>
		               			<c:otherwise>"<c:out value='${netItem.dt}'/>"</c:otherwise>
               				</c:choose>
		      			</c:forEach>
		               ],
		      datasets: [
				<c:forEach var="vmItem" items="${vmList }" varStatus="i">
					<c:if test="${i.count>1}">
					...
					</c:if>
						{
					          label:"<c:out value='${vmItem.vm_comp_id}'/>",
					          data: [
									<c:forEach var="netItem" items="${netList }" varStatus="j">
										<c:if test="${j.count>1}">	,		</c:if>
										<c:set var="useRt" value="${netItem[vmItem.vm_seq]}"/>
										<c:choose>
											<c:when test="${empty netItem[vmItem.vm_seq]}">""</c:when>
											<c:otherwise><c:out value='${useRt}'/></c:otherwise>
										</c:choose>
    ...
    setChartColor(netAreaChartData,'line');
    ...
    netAreaChartData.datasets.forEach(function(label, i){
		var arealegendItem = $('<span></span>').append('<i></i>' + label['label']);
		arealegendItem.find('i').css('background', netAreaChartData.datasets[i].strokeColor);
		$('#netlineChart ~ .chartLegend').append(arealegendItem);
	});
    ...
    var netLineChartCanvas = $("#netlineChart").get(0).getContext("2d");
    var netLineChart = new Chart(netLineChartCanvas);
    lineChartOptions.scaleStepWidth=Math.floor(fn_getMaxScaleStepWidth(maxNetUseRt)/areaChartOptions.scaleSteps);
    netLineChartLine = netLineChart.Line(netAreaChartData, lineChartOptions);
  ...
  //10분단위일경우 시간단위로만 label을 출력함.
    <c:if test="${searchVo.colctCd eq 'MI'}">
	    ...
	    for(var i=0; i<netAreaChartData.labels.length; i++){
	    	if(netAreaChartData.labels[i].substring(3,5)!='00'){
	    		netAreaChartData.labels[i]="";
	    	}
	    }
    </c:if>
...
$(window).resize(function() {
    ...
    if (netLineChartLine != null) netLineChartLine.destroy();
    window.setTimeout(function() {
    	$('#cpulineChart, #memlineChart, #sanlineChart, #netlineChart').height('200px');
    	ChartDraw();
    }, 200);
});
...
<form:form commandName="searchVo" method="get">
	<c:choose>
		<c:when test="${ searchVo.jobId ne null or !empty searchVo.jobId }">
			...
			<div class="box-body ">
				<div class="form-group">
		           ...
		           <c:choose>
			           ...
			           <div class="cell-body">
		             		<form:input path="jobNm" cssClass="form-control input-sm" maxlength="" title="업무명" readonly="true"/>
		             	</div>
		            </c:when>
		            <c:otherwise>
		               <div class="cell-title">
			           ...
			           <div class="cell-body">
		             		<form:input path="vmNm" cssClass="form-control input-sm" maxlength="" title="가상서버명" readonly="true"/>
		             	</div>
		            </c:otherwise>
		           </c:choose>
				   </div>
...
<div class="col-box-100 detail-col">
        ...
        <div class="box">
         ...
         <div class="box">
          <div class="box-header">
              <h3 class="box-title">네트워크 사용량</h3>
          </div><!-- /box-header -->
          <div class="box-body collapse in" id="graph-b">
            <div class="chart">
            <div>(KB/s)</div>
              <canvas id="netlineChart" style="height:200px;"></canvas>
              <div class="chartLegend"></div>
            </div>
          </div><!-- /box-body -->
        </div><!-- /box(Line Chart) -->
              ...
              <caption>CPU 사용률</caption>
                ...
                <thead>
                	<tr>
                		<th>기간</th>
                  		<c:forEach var="vo" items="${vmList }" varStatus="i">
							<th title="<c:out value="${vo.vm_comp_id}"/>"><c:out value="${vo.vm_comp_id}"/></th>
                  		</c:forEach>
                	</tr>
                </thead>
                <tbody>
        ...
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">네트워크 사용률</h3>
            <div class="box-tools">
              <div class="input-group pull-right">
                <div class="input-group-select">
                </div>
          ...
          <div class="box-body no-padding">
              <table class="table table-hover table-vertical table-bordered table-layout-fixed">
                ...
                <c:forEach var="map" items="${netList }" varStatus="i">
                	<tr>
	                  <td class="ellipsis"><c:out value="${map['dt']}"/></td>
		              <c:forEach var="vmItem" items="${vmList }">
		              	<c:set var="vmSeq" value="${vmItem.vm_seq}"/>
...
<script>
      ...
      function excelDown(){
    	 <c:if test="${vmList eq null or empty vmList }">
    		...
    		searchVo.action='<c:url value="selectJobTimeResUseRtPXlsDown.do" />';
    		searchVo.submit();
    	}
</script>