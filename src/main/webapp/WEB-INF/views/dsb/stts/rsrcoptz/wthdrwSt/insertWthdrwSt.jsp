	...
	<div class="col-box-100 detail-col">
		<div class="detail-list-box box">
			<div class="box-header">
				<h3 class="box-title">자원 회수 현황</h3>
				<div class="box-tools">
						...
						<c:choose>
									...
									<tr>
										...
										<td><select name="stdrYr" id="stdrYr" class="form-control input-sm" title="년도" disabled>
												<c:forEach var="yearVo" items="${yearCdList }">
													<option value="${yearVo.cd}" <c:if test='${yearVo.cd eq vo.stdrYr}'>selected</c:if>><c:out value="${yearVo.cdNm }" /></option>
												</c:forEach>
											</select>
										</td>
							...
							<c:otherwise>
								<tr>
									...
									<td class="alignR">
										<input type="text" name="returnReqQtyVcore" id="returnReqQtyVcore" maxlength="5" class="col55 alignR essential onlyNumber" title="반납요구량vCore">
									</td>
									<td class="alignR">
										<input type="text" name="returnReqQtyMem" id="returnReqQtyMem" maxlength="5" class="col55 alignR essential onlyNumber" title="반납요구량MEM">
									</td>
									<td class="alignR">
...
<table id="tmplTable" style="display: none">
	<tr>
		...
		<td class="alignR">
			<input type="text" name="returnReqQtyVm" id="returnReqQtyVm" maxlength="5" title="반납요구량가상서버" class="col55 essential onlyNumber alignR">
		</td>
		<td class="alignR">
...
<script type="text/javascript">
	...
	function fn_addTr() {
		...
		$('[name=rsrcPoolNm]', '#tableWthdrw tr:last').val("");//추가한 행의 기관명 삭제
		$('[name=rsrcPoolId]', '#tableWthdrw tr:last').val("");//추가한 행의 기관ID 삭제
	...
	function fn_doDelete() {
		...
		jConfirm("자원 회수 현황을 삭제하시겠습니까?", function() {
			var options = {
				url : '<c:url value="deleteWthdrwSt.do" />',
				type : 'POST',
				dataType : 'json',
	...
	function fn_doInsert() {
		...
		console.log(JSON.stringify(submitData));
		jConfirm("자원 회수 현황을 저장하시겠습니까?", function() {
			var options = {
				url : '<c:url value="insertWthdrwSt.do" />',
				data : JSON.stringify(submitData),
				type : 'POST',
				dataType : 'json',
				contentType : "application/json;charset=UTF-8",
