	...
	<div class="box list-box">
		...
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical">
				...
				<tbody>
					<c:choose>
						<c:when test="${memUseRtList eq null or empty memUseRtList }">
						...
						<c:otherwise>
							<c:forEach var="vo" items="${memUseRtList }" varStatus="status1">
								<tr>
									<c:if test="${status1.index == 0 }">
										<td rowspan="7">MEM(GB)</td>
									</c:if>
									<td>
										<c:choose>
	...
	<div class="box list-box">
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical">
				...
				<tbody>
					<c:choose>
						<c:when test="${topMemUseRtList eq null or empty topMemUseRtList }">
							<c:if test='${searchVo.regionId eq null }'>
								<tr>
									<td colspan="13">조회 버튼을 클릭하시기 바랍니다.</td>
								</tr>
						...
						<c:otherwise>
							<c:forEach var="vo" items="${topMemUseRtList }" varStatus="status">
								<tr>
...
<script type="text/javascript">
	function fn_excelDown() {
		searchVo.action = '<c:url value="selectAxMemUseRtXlsDown.do" />';
		searchVo.submit();
	}
	function fn_search() {
		searchVo.action = '<c:url value="selectAxMemUseRtList.do" />';
		searchVo.submit();
	}
	//검색조건 초기화
	function fn_reset() {
