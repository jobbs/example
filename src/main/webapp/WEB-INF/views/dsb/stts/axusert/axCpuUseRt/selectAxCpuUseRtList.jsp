	...
	<div class="box list-box">
		...
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical">
				...
				<tbody>
					<c:choose>
						<c:when test="${axCpuUseRtList eq null or empty axCpuUseRtList }">
							<c:if test='${searchVo.regionId eq null }'>
								<tr>
									<td colspan="13">조회 버튼을 클릭하시기 바랍니다.</td>
								</tr>
							</c:if>
						...
						<c:otherwise>
							...
							<c:forEach var="vo" items="${axCpuUseRtList }" varStatus="status1">
								<tr>
									<c:if test="${status1.index == 0 }">
										<td rowspan="10">Core</td>
									</c:if>
									<td><c:out value="${vo.capa }" /></td>
	...
	<div class="box list-box">
		<div class="box-body no-padding list-body">
			<table class="table table-hover table-vertical">
				...
				<tbody>
					<c:choose>
						<c:when test="${axTopCpuUseRtList eq null or empty axTopCpuUseRtList }">
							<c:if test='${searchVo.regionId eq null }'>
								<tr>
									<td colspan="13">조회 버튼을 클릭하시기 바랍니다.</td>
								</tr>
						...
						<c:otherwise>
							<c:forEach var="vo" items="${axTopCpuUseRtList }" varStatus="status">
								<tr>
...
function fn_search() {
	...
	$("#frm").attr("action", "<c:url value='selectAxCpuUseRtList.do'/>");
	$("#frm").submit();
	return false;
}
...
function excelDown(){
	if(${axCpuUseRtList eq null or empty axCpuUseRtList}) {
		jAlert("엑셀로 다운로드할 데이터가 없습니다.");
		return;
	}

	$("#frm").attr("target", "_self");
	$("#frm").attr("action", "<c:url value='selectAxCpuUseRtXlsDown.do'/>");
	$("#frm").submit();
	return false;
}
</script>