...
<script type="text/javascript">
				...
				.dynatree(
						{
							...
							initAjax : {
								url : '<c:url value="/cmn/component/dsb/selectRegionToPmTreeByOnnara.do?pmTree=main"/>'
				...
				.dynatree(
						{
							...
							initAjax : {
								url : '<c:url value="/cmn/component/dsb/selectInstitutionToVmTreeByOnnara.do?vmTree=main"/>'
							},
							onPostInit : function(isReloading, isError) {
								if(isError){
	...
	function fn_inition() {
		...
		<c:choose>
			<c:when test = "${mainRsrcView}">
				if (dynSelect == '') {
					$.get(
									'<c:url value="/cmn/component/dsb/selectPmIdxListByOnnara.do"/>'+"?"+param,
									{
										'id' : 'NIRS'
			...
			<c:otherwise>
				...
				if (dynSelect == '') {
					$.get(
									'<c:url value="/cmn/component/dsb/selectVmIdxListByOnnara.do"/>'+"?"+param,
									{
										'id' : 'NIRS'
									}, loadSuccessHandler, 'html');
				}
	...
	function loadSelectDetail(key) {
		var url = '<c:url value="/cmn/component/dsb/selectPmIdxListByOnnara.do"/>'+"?"+param;
		var id = null;
		var vmPod = null;
		if (key) {
			id = key;
	...
	function loadSelectDetail_inc(key, gubun) {
		var url = '<c:url value="/cmn/component/dsb/selectVmIdxListByOnnara.do"/>'+"?"+param;
		var id = null;
		var vmPod = null;
