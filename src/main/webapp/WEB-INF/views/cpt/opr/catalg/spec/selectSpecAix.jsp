...
<form:form id="selectSpecAix" commandName="resultVo" action="updateSpec.do">
				...
				<table class="table table-horizon">
					...
					<tbody>
					  	...
					  	<tr>
							...
							<td>
								<form:input path="entMaxVl" type="number" class="form-control essential onlyFloat" value="${resultVo.entMaxVl }" step="0.01" min="0.01" max="1.00" onblur="fn_checkData();" maxlength="4" title="Entitlement 최대값"/>
							</td>
						</tr>
						<tr>
						  <th>
							...
							<td>
								<form:input path="entDfltVl" type="number" class="form-control essential onlyFloat" value="${resultVo.entDfltVl }" step="0.01" min="0.01" max="1.00" onblur="fn_checkData();" maxlength="4" title="Entitlement 기본값"/>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td>
								<form:input path="entMinVl" type="number" class="form-control essential onlyFloat" value="${resultVo.entMinVl }" step="0.01" min="0.01" max="1.00" onblur="fn_checkData();" maxlength="4" title="Entitlement 최소값"/>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table class="table table-horizon">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<form:input path="vcoreMinVl" type="number" class="form-control essential onlyInteger" value="${resultVo.vcoreMinVl }" step="1" min="1" max="128" onblur="fn_checkData();" maxlength="3" title="vCore 최소값"/>
							</td>
						</tr>
						<tr>
							<th>
								<div class="input-group input-group-check">
									<input id="uncappWeightCheck" type="checkbox" onclick="fn_changeUncappWeight()" title="Uncapped Weight 설정여부">
									<label for="uncappWeightCheck">Uncapped Weight</label>
								</div>
							</th>
							<td>
								<form:input path="uncappWeight" name="uncappWeightText" type="number" class="form-control onlyFloat" value="${resultVo.uncappWeight }" disabled="true" step="0.01" min="0.01" max="128.00" onblur="fn_checkData();" maxlength="5" title="Uncapped Weight"/>
							</td>
						</tr>
					</tbody>
				</table>
					...
					<tbody>
					  ...
					  <tr>
					  	<th>
							...
							<td>
								<form:input path="memMinVl" type="number" class="form-control essential onlyInteger" value="${resultVo.memMinVl }" step="1" min="1" max="256" onblur="fn_checkData();" maxlength="3" title="메모리 최소값"/>
							</td>
							<td>GB</td>
						</tr>
					</tbody>
				...
				<table class="table table-horizon">
					...
					<tbody>
						<tr>
							...
							<td>
								<form:input path="strgDfltVl" type="number" class="form-control essential onlyInteger" value="${resultVo.strgDfltVl }" step="1" min="1" max="9999999" onblur="fn_checkData();" maxlength="7"  title="스토리지 기본값"/>
							</td>
							<td>GB</td>
						</tr>
					</tbody>
...
<script type="text/javascript">
	...
	function doSubmit(result){
		...
		if(!fn_form_validation("selectSpecAix")){
			return;
		}
		...
		if($('#entMaxVl').val() != document.querySelector(('#entMaxVl')).defaultValue){
			bool = true;
		}
		if($('#entDfltVl').val() != document.querySelector(('#entDfltVl')).defaultValue){
			bool = true;
		}
		if($('#entMinVl').val() != document.querySelector(('#entMinVl')).defaultValue){
			bool = true;
		}
		...
		if($('#vcoreMinVl').val() != document.querySelector(('#vcoreMinVl')).defaultValue){
			bool = true;
		}
		if($('#uncappWeight').val() != document.querySelector(('#uncappWeight')).defaultValue){
			bool = true;
		}
		...
		if($('#memMinVl').val() != document.querySelector(('#memMinVl')).defaultValue){
			bool = true;
		}
	...
	function fn_updateSpec(){
		...
		$('#selectSpecAix').ajaxSubmit(options);
	}
