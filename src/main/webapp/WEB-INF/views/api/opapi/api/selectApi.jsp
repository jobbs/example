...
<div class="col-box-100 detail-col">
	<form:form commandName="apiVo" enctype="multipart/form-data">
		<form:hidden path="keyId" value="${apiVo._id}"/>
		<form:hidden path="rev" value="${apiVo._rev}"/>
				...
				<table class="table table-horizon">
					...
					<tbody>
             			...
             			<tr>
    						...
    						<td>
								<nform:selectCode parentCd="203"
					              	parentGrpCd="041"
					              	name="apiVerCd"
					              	id="apiVerCd"
					              	wholeText="선택"
					              	title="버전"
					              	value="${apiVo.apiVerCd}"
  								...
  								<div class="form-group mg-bottom-5">
  									<table class="table table-bordered" id="cnvrRoleTable">
										...
										<tbody id="cnvrRoleListTBody">
                  						<!-- 추가되는 부분 시작-->
											<c:forEach var="cnvrRulesVo" items="${apiVo.cnvrRules}" varStatus="i">
											<tr>
												<td>
													<input type="checkbox" name="delKey" class="delKey" value="" title="항목선택"/>
												</td>
												<td>
													<input type="text"title="SEQ" name="cnvrRules[].targtSeq" value="${cnvrRulesVo.targtSeq}" class="form-control onlyNumber essential" />
												</td>
												<td>
													<nform:selectCode
														parentCd="202"
													 	parentGrpCd="040"
													 	name="cnvrRules[].targtMethodCd"
													 	wholeText="선택"
													 	value="${cnvrRulesVo.targtMethodCd}"
													 	cssClass="form-control"
													 	title="Method" />
												</td>
												<td>
													<input type="text" title="URI" name="cnvrRules[].targtUri" value="${cnvrRulesVo.targtUri}" class="form-control essential" />
												</td>
												<td>
													<nform:selectCode
														parentCd="${apiVo.stackClCd}"
													 	parentGrpCd="039"
													 	name="cnvrRules[].mngrClCd"
													 	value="${cnvrRulesVo.mngrClCd}"
													 	title="매니저"
													 	wholeText="선택"
													 	cssClass="form-control essential"
													 	onchange="selectboxMngVer(this, ${i.count-1})"/>
												</td>
												<td>
													<nform:selectCode
														parentCd="${cnvrRulesVo.mngrClCd}"
													 	parentGrpCd="039"
													 	id="apiVerCd${i.count-1}"
													 	name="cnvrRules[].apiVerCd"
													 	title="버전"
													 	wholeText="선택"
													 	value="${cnvrRulesVo.apiVerCd}"
													 	cssClass="form-control essential" />
												</td>
												<td>
													<div class="file-form-field input-group">
														<button type="button" class="btn btn-function btn-sm" id="btnCnvrScript${i.count-1}" name="btnCnvrScript" onclick="fn_popupModal(${i.count-1});">변환 룰</button>
														<textarea id="cnvrScript${i.count-1}" name="cnvrRules[].cnvrScript" title="변환룰" class="form-control essential" style="display:none">${cnvrRulesVo.cnvrScript}</textarea>
													</div>
												</td>
											</tr>
											</c:forEach>
  								...
  								<div class="form-group mg-bottom-5">
  									<table class="table table-bordered" id="reqParamTable">
										...
										<tbody id="reqParamListTBody">
                  						<!-- 추가되는 부분 시작-->
											<c:forEach var="reqParamsVo" items="${apiVo.reqParams}" varStatus="i">
											<tr>
												<td>
													<input type="checkbox" name="reqDelKey" class="delKey" value="" title="항목선택"/>
												</td>
												<td>
													<nform:selectCode
														parentCd="REQTYPE"
													 	parentGrpCd="080"
													 	name="reqParams[].reqParamClCd"
													 	whole="true"
													 	title="분류"
													 	wholeText="선택"
													 	value="${reqParamsVo.reqParamClCd}"
													 	cssClass="form-control essential" />
												</td>
												<td>
													<input type="text" title="항목" name="reqParams[].reqParamIemId" value="${reqParamsVo.reqParamIemId}" class="form-control essential" />
												</td>
												<td>
													<input type="text" title="항목명" name="reqParams[].reqParamIemNm" value="${reqParamsVo.reqParamIemNm}" class="form-control essential" />
												</td>
												<td>
													<nform:selectCode
														parentCd="TYPE"
													 	parentGrpCd="081"
													 	name="reqParams[].reqParamType"
													 	whole="false"
													 	title="타입"
													 	value="${reqParamsVo.reqParamType}"
													 	cssClass="form-control essential"  />
												</td>
												<td>
													<nform:selectCode
														parentCd="ESSNTL"
													 	parentGrpCd="079"
													 	name="reqParams[].reqParamEssntlAt"
													 	whole="false"
													 	title="필수여부"
													 	value="${reqParamsVo.reqParamEssntlAt}"
													 	cssClass="form-control essential" />
												</td>
												<td>
													<input type="text" title="설명" name="reqParams[].reqParamDc" value="${reqParamsVo.reqParamDc}" class="form-control essential" />
												</td>
											</tr>
											</c:forEach>
										<!-- 추가되는 부분 끝-->
  									...
  									<table class="table table-bordered" id="rspnsResultTable">
										...
										<tbody id="rspnsResultListTBody">
                  						<!-- 추가되는 부분 시작-->
											...
											<tr>
												<td>
													<input type="checkbox" name="rspnsDelKey" class="delKey" value="" title="항목선택"/>
												</td>
												<td>
													<input type="text" title="항목" name="rspnsResults[].rspnsResultIemId" value="${rspnsResultVo.rspnsResultIemId}" class="form-control essential" />
												</td>
												<td>
													<input type="text" title="항목명" name="rspnsResults[].rspnsResultIemNm" value="${rspnsResultVo.rspnsResultIemNm}" class="form-control essential" />
												</td>
												<td>
													<nform:selectCode
														parentCd="TYPE"
													 	parentGrpCd="081"
													 	name="rspnsResults[].rspnsResultType"
													 	title="타입"
													 	whole="false"
													 	value="${rspnsResultVo.rspnsResultType}"
													 	cssClass="form-control essential" />
												</td>
												<td>
													<nform:selectCode
														parentCd="ESSNTL"
													 	parentGrpCd="079"
													 	name="rspnsResults[].rspnsResultEssntlAt"
													 	title="필수여부"
													 	whole="false"
													 	value="${rspnsResultVo.rspnsResultEssntlAt}"
													 	cssClass="form-control essential" />
												</td>
												<td>
													<input type="text" title="설명" name="rspnsResults[].rspnsResultDc" value="${rspnsResultVo.rspnsResultDc}" class="form-control essential" />
												</td>
											</tr>
											</c:forEach>
										<!-- 추가되는 부분 끝-->
						...
						<tr>
  							...
  							<td colspan="3">
  								<form:textarea path="dc" class="form-control" rows="3" maxlength="4000" title="설명"/>
                			</td>
              			</tr>
           			</tbody>
    			</table>
...
<div class="col-box-100 detail-col">
	...
	<div class="edit-btn-group">
		<c:url var="listUrl" value="selectApiList.do">
		...
		<div class="pull-right btns">
			<menu:authorize>
			<button type="button" class="btn btn-hover-green" data-toggle="tooltip" data-original-title="저장" onclick="fn_updateApi();">
				<i class="fa fa-check"></i>
			</button>
			<button type="button" class="btn btn-hover-red" data-toggle="tooltip" data-original-title="삭제" onclick="jConfirm('Api를 삭제하시겠습니까?', fn_deleteApi)">
...
var mngrRowCnt = "${apiVo.cnvrRules.size()}";
var allowExt = ["txt"];
...
//Api수정 처리
function fn_updateApi(){
	if(!fn_form_validation("apiVo")){
		return;
	}
	...
	jConfirm('Api를 수정하시겠습니까?', function(){
		var options = {
			type: 'post',
			dataType: 'json',
			url : contextPath+'/api/opapi/api/updateApi.do',
			success: fn_pageMove,
			beforeSend : function() {
...
function fn_deleteApi(){
	...
	$.ncmsLoding.startFullScreen();
	$.post('deleteApi.do', $('#apiVo').serialize(), fn_pageMove2, 'json').always(function() {
		$.ncmsLoding.remove();
	 });
}
...
function fn_insertCnvrRule() {
	...
	var html =''
			...
			+'</td>'
			+'<td>'
			+'		<input type="text" title="SEQ" name="cnvrRules[].targtSeq" value="" class="form-control onlyNumber essential" />'
			+'</td>'
			+'<td>'
			+'		<select name="cnvrRules[].targtMethodCd" title="Method" class="form-control essential">'
			+'		</select>'
...
function fn_deleteCnvrRole(){
	...
	if( checked == 0 ) {
		...
		parent.jAlert("변환룰은 1개이상은 존재해야 합니다.");
		return;
	}
