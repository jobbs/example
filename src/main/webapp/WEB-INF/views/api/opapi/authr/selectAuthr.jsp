...
<div class="col-box-100 detail-col">
	<form:form commandName="authrVo">
		<form:hidden path="authrId" />
		<form:hidden path="rev" />
								...
								<div class="box-header">
									...
									<div class="box-tools input-group-box">
										...
										<div class="input-group-cell">
											<div class="input-group">
												<input type="text" name="searchNm" id="searchNm" class="form-control input-sm" title="검색">
								...
								<div class="box-body no-padding" style="max-height: 250px; min-height: 30px; overflow-y: auto; overflow-X: hidden;">
									...
									<caption>할당 OpenAPI</caption>
										...
										<tbody id="targetTbody">
											<c:choose>
											 	<c:when test="${authrVo.apiMapng eq null or empty authrVo.apiMapng }">	 -->
													<tr>
														<td colspan="2">검색된 데이터가 없습니다.</td>
													</tr>
											 	</c:when>
												<c:otherwise>
													<c:forEach var="vo" items="${authrVo.apiMapng }" varStatus="i">
														<tr onclick='fn_checkUser(this)'>
															<td><input type="checkbox" alt="${fn:substringAfter(vo,'&')}" name="apiMapng" value="${vo}" title="항목선택"></td>
...
<div class="col-box-100 detail-col">
	...
	<div class="edit-btn-group">
		<c:url var="listUrl" value="selectAuthrList.do">
			<c:forEach var="curParam" items="${param}">
				<c:if test="${curParam.key ne 'authrId'}">
		...
		<div class="pull-right btns">
			<menu:authorize>
			<button type="button" class="btn btn-hover-green" data-toggle="tooltip" data-original-title="저장" onclick="fn_updateAuthr();">
				<i class="fa fa-check"></i>
			</button>
			<button type="button" class="btn btn-hover-red" data-toggle="tooltip" data-original-title="삭제" onclick="jConfirm('OpenApi권한을 삭제하시겠습니까?', fn_deleteAuthr)">
...
function fn_searchUser() {
	...
	$.post(
		'<c:url value="/api/opapi/authr/selectOpenApiList.do"/>',
		{
					...
					for (var i = 0; len > i; i++) {
						...
						if (eqChk) {
							str += "<tr onclick='fn_checkUser(this)'>"
								+ "<td><input type='checkbox' name='opApiUri' title='항목선택' alt='"+result.data[i].uri
								+ "' value='"+result.data[i].opApiId+"&"+result.data[i].uri+"'/></td>"
...
function fn_allRight() {
	...
	$('input:checkbox', '#searchTable').each(
		function() {
			var id = this.value;
			var isOK = true;
			//debugger;
			...
			if (isOK) {
				var html = '<tr onclick="fn_checkUser(this)"><td><input type="checkbox" alt="'
						+ $(this).attr("alt")
						+ '"  name="apiMapng" title="항목선택" value="'
						+ $(this).val()
						+ '"></td><td>'
						+ $(this).attr("alt") + '</td></tr>'
...
function fn_right() {
	...
	$('input:checkbox:checked', '#searchTable')
			.each(
					function() {
						var id = this.value;
						var isOK = true;
...
function fn_left() {
	...
	$('input:checkbox:checked', '#dspthTable')
			.each(
					function() {
						var id = this.value;
						var isOK = true;
						...
						if (isOK) {
							...
							var html = '<tr onclick="fn_checkUser(this)"><td><input type="checkbox" alt="'
									+ $(this).attr("alt")
									+ '" name="opApiUri" title="항목선택" value="'
									+ $(this).val()
									+ '"></td><td>'
									+ $(this).attr("alt") + '</td></tr>'
			...
			.each(
					function() {
						...
						if (isOK) {
							var html = '<tr onclick="fn_checkUser(this)"><td><input type="checkbox" alt="'
									+ $(this).attr("alt")
									+ '" name="opApiUri" titile="항목선택" value="'
									+ $(this).val()
									+ '"></td><td>'
									+ $(this).attr("alt") + '</td></tr>'
							$('#searchTable tbody').append(html);
...
function fn_updateAuthr() {
	...
	// 입력 validation 체크
	if (!fn_form_validation("authrVo")) {
		return;
	}

	jConfirm('OpenApi권한을 수정하시겠습니까?', function(){
		$.ncmsLoding.startFullScreen();
		$.post('updateAuthr.do', $('#authrVo').serialize(), fn_pageMove,'json').always(function() { $.ncmsLoding.remove(); });
	});
}
...
function fn_deleteAuthr() {
	$.ncmsLoding.startFullScreen();
	$.post('deleteAuthr.do', $('#authrVo').serialize(), fn_pageMove2,'json').always(function() { $.ncmsLoding.remove(); });
}
