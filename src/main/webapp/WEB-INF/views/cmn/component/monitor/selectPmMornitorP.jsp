			...
			<div class="box-body ">
				<div class="form-group">
		           <div class="form-cell form-cell-100">
			           ...
			           <div class="cell-body"><b><c:choose><c:when test="${ null eq pmInfo.pmCompId }"><c:out value="${pmInfo.pmNm }"/></c:when><c:otherwise><c:out value="${pmInfo.pmNm } ( ${pmInfo.pmCompId } )"/></c:otherwise></c:choose></b></div>
				   </div>
...
<script>
    ...
    function excelDown(){
    	<c:if test="${chartList eq null or empty chartList }">
    		...
    		searchVo.action='<c:url value="selectPmMornitorPXlsDown.do" />';
    		searchVo.submit();
    }
    function fn_search(){
    	searchVo.action='<c:url value="selectPmMonitorP.do" />';
  		searchVo.submit();
    }
