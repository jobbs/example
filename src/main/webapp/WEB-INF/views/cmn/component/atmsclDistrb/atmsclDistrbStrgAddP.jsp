				...
				<colgroup>
					...
					<tbody>
						<tr>
							...
							<td colspan="3">
								<select id="strgPvNm" name="strgPvNm" class="form-control input-sm essential" onchange="changePvInfo();">
			            ...
			            <tr>
							...
							<td>
								<input type="text" name="mountPath" id="mountPath" class="form-control essential" onkeypress="numCheck(this);" placeholder="example: /data" title="마운트 경로" />
							</td>
...
<script type="text/javascript">
    ...
    function strgAdd(){
    	...
    	if (mountPathCheck == "" || mountPathCheck == "undefined") {
    		jAlert("마운트 경로를 입력해 주시기 바랍니다.");
    		return;
    	}else if("/" != mountPathCheck[0] ){
    		jAlert("마운트 경로를 확인해 주시기 바랍니다.");
    		return;
    	}else if(!regId2.test(mountPathCheck) ){
    		jAlert("마운트 경로는 영문/숫자(-,_)만 가능합니다.");
    		return;
    	}

		if (volumNmCheck == "" || volumNmCheck == "undefined") {
			jAlert("볼륨명을 입력해 주시기 바랍니다.");
    		return;
    	}else if(!regId.test(volumNmCheck)){
			jAlert("볼륨명은 영문과 숫자만 입력해 주시기 바랍니다.");
    		return;
		}
	   ...
	   jConfirm('스토리지 할당 하시겠습니까?', function(){
			...
			$('#atmsclDistrbVo').attr('action', contextPath+'/cpt/rsrc/atmscl/atmsclDistrb/insertPvcStrgAdd.do');
